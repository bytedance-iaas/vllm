...
def test_use_index_cache_false_disables_freq_skip():
    flags = compute_dsv4_index_cache_skip_flags(
        COMPRESS_RATIOS,
        NUM_HIDDEN_LAYERS,
...
def test_use_index_cache_false_disables_pattern_skip():
    flags = compute_dsv4_index_cache_skip_flags(
        COMPRESS_RATIOS,
        NUM_HIDDEN_LAYERS,
        use_index_cache=False,
...
def test_skipped_indexer_weight_filter_matches_only_omitted_indexers():
    ...
    assert not dsv4_nvidia_model._is_dsv4_skipped_indexer_weight(
        "model.layers.2.attn.indexer.wk_weights_proj.weight",
        skipped_layer_ids,
    )
    assert not dsv4_nvidia_model._is_dsv4_skipped_indexer_weight(
