...
def test_humming_w4a8_selection_uses_standard_activation_format():
    ...
    with patch.object(w4a8, "backend_to_kernel_cls", return_value=[StandardExperts]):
...
def test_humming_w4a8_rejects_missing_activation_params():
    config = make_dummy_moe_config(activation=MoEActivation.SWIGLUOAI_UNINTERLEAVE)
...
def test_humming_w4a8_activation_preserves_explicit_zero_params():
    ...
    assert captured["activation"] == MoEActivation.SWIGLUOAI_UNINTERLEAVE
    assert captured["input"] is input_tensor
    assert captured["output"] is output_tensor
    assert captured["clamp_limit"] == 7.0
    assert captured["alpha"] == 0.0
    assert captured["beta"] == 0.0
