...
def test_save_partial_states_supports_mixed_state_dtype(state_dtype: torch.dtype):
    head_dim = 512
    block_size = 8
    compress_ratio = 128
    num_tokens = 4
    ...
    state_cache = torch.zeros(
        1, block_size, 2 * head_dim, dtype=state_dtype, device=device
    )
    ...
    for token_idx in (0, 1, 3):
        torch.testing.assert_close(
            state_cache[0, token_idx, :head_dim],
