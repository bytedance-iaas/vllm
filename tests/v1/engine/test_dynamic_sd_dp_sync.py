...
from vllm.v1.engine.utils import (
    ...
    needs_dp_collective_config_validation,
...
def test_dynamic_sd_dp_idle_rank_still_enters_pressure_sync():
    core = DPEngineCoreProc.__new__(DPEngineCoreProc)
    core._handle_shutdown = Mock(side_effect=[True, False])
    core._process_input_queue = Mock()
    core._maybe_publish_request_counts = Mock()
    core._process_engine_step = Mock(return_value=False)
    core._sync_dynamic_sd_batch_size_override = Mock()
    core.scheduler = Mock()
    ...
    with pytest.raises(SystemExit):
        DPEngineCoreProc.run_busy_loop(core)

    core._sync_dynamic_sd_batch_size_override.assert_called_once_with()
...
def test_feature_off_dummy_batch_calls_executor_without_runtime_k():
...
def test_feature_off_scheduler_dummy_batch_runtime_k_is_none():
    scheduler = create_scheduler(num_speculative_tokens=0)

    assert scheduler.get_num_spec_tokens_to_schedule_for_dummy_batch() is None


def test_static_spec_scheduler_dummy_batch_runtime_k_is_none():
    scheduler = create_scheduler(num_speculative_tokens=5)

    assert scheduler.get_num_spec_tokens_to_schedule_for_dummy_batch() is None


def test_static_dspark_dummy_batch_calls_executor_without_runtime_k():
    core = EngineCore.__new__(EngineCore)
    core.vllm_config = SimpleNamespace(
        use_v2_model_runner=True,
        speculative_config=SimpleNamespace(
...
def test_dp_collective_hash_covers_dynamic_sd_collective_config():
    ...
    assert _dp_collective_hash(method="dflash") != base_hash
    assert _dp_collective_hash(max_k=5) != base_hash
    assert _dp_collective_hash(async_scheduling=False) != base_hash
    assert _dp_collective_hash(schedule=[(1, 8, 3), (9, 16, 0)]) != base_hash


def test_dense_dp_dynamic_sd_requires_collective_config_validation():
    config = SimpleNamespace(
    ...
    assert needs_dp_collective_config_validation(config, coordinated_dp=False)
