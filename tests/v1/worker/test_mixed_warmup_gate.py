    ...
    runner = SimpleNamespace(
        ...
        speculative_config=(
            ...
            if method is None
                ...
                method=method,
                uses_dynamic_speculative_decoding=lambda: dynamic_sd,
            )
        ),
...
def test_warmup_kernels_cleans_generic_before_parallel_draft_and_restores_k(
    ...
    runner = _make_warmup_runner(method, dynamic_sd=True)
...
def test_warmup_kernels_restores_k_and_cleans_parallel_req_on_failure(monkeypatch):
    monkeypatch.setattr(torch.accelerator, "synchronize", lambda: None)
    runner = _make_warmup_runner("dflash", dynamic_sd=True)
    active_req_ids: set[str] = set()
...
def test_warmup_kernels_runs_parallel_draft_full_k_for_dynamic_dflash_dspark(
    monkeypatch,
):
    monkeypatch.setattr(torch.accelerator, "synchronize", lambda: None)
    ...
    for method in ("dflash", "dspark"):
        ...
        warmup_kernels(
            runner,
            worker_execute_model=lambda output, execute_calls=execute_calls: (
                _record_execute_call(execute_calls, output)
            ),
            worker_sample_tokens=lambda grammar, sample_calls=sample_calls: (
                _record_sample_call(sample_calls, grammar)
            ),
        )
        ...
        assert execute_calls[9].finished_req_ids == {"_warmup_parallel_draft_full_k_0_"}


@pytest.mark.parametrize("method", ["dflash", "dspark", "eagle3"])
def test_warmup_kernels_skips_parallel_draft_full_k_without_dynamic_sd(
    monkeypatch,
    method,
):
    monkeypatch.setattr(torch.accelerator, "synchronize", lambda: None)
    runner = _make_warmup_runner(method, dynamic_sd=False)
    execute_calls: list = []
    sample_calls: list = []
