...
def test_compressor_builder_rejects_online_c128_with_pcp():
    manager = PCPManager(
        pcp_world_size=2,
        pcp_rank=0,
        max_buffer_num_tokens=64,
    ...
    builder = object.__new__(CompressorMetadataBuilder)
    builder._online_c128 = True

    with pytest.raises(NotImplementedError, match="Online C128"):
        builder.build(0, _make_pcp_common_metadata(manager))
