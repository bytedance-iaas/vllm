...
def _make_replicated_draft_proposer_for_validation(
    *,
    connector: str | None = "MooncakeConnector",
...
def test_replicated_draft_dcp1_allows_local_prefill_without_connector() -> None:
    proposer = _make_replicated_draft_proposer_for_validation(
        connector=None,
        dcp_size=1,
    )
...
def test_replicated_draft_dcp2_rejects_local_prefill_without_connector() -> None:
    proposer = _make_replicated_draft_proposer_for_validation(connector=None)

    with pytest.raises(ValueError, match="Mooncake remote prefill"):
        proposer._validate_replicated_draft_kv_config()
...
def test_attention_spec_merge_rejects_mixed_temporal_layouts() -> None:
    ...
    with pytest.raises(ValueError, match="same temporal layout"):
