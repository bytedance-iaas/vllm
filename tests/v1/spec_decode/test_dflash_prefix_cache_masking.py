...
def test_set_draft_query_padding_mask_marks_active_rows_and_graph_padding():
    input_buffers = _make_input_buffers()

    mask = _set_draft_query_padding_mask(
        input_buffers,
        num_query_tokens=5,
        num_tokens_padded=8,
        dummy_run=False,
    ...
    assert mask.tolist() == [False, False, False, False, False, True, True, True]


def test_set_draft_query_padding_mask_marks_idle_dummy_rows_as_padding():
    input_buffers = _make_input_buffers()
    input_buffers.is_padding[:8].fill_(False)
...
def test_dflash_run_model_passes_padding_slice_to_forward_context(monkeypatch):
    ...
    torch.testing.assert_close(
        captured["is_padding"], speculator.input_buffers.is_padding[:8]
    )
