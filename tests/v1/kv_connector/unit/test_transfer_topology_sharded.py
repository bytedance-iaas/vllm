...
class _UnexpectedBackend:
    ...
    def get_kv_cache_shape(*args, **kwargs):
        raise AssertionError("backend should not be probed")
...
def test_standard_topology_stops_after_first_standard_backend() -> None:
    topology = TransferTopology(
        tp_rank=0,
        tp_size=4,
        block_size=16,
        ...
        is_mamba=False,
        total_num_kv_heads=8,
        attn_backends=[_FakeAttentionBackend, _UnexpectedBackend],
    )

    assert topology.local_physical_heads == 2


def test_mla_topology_only_probes_first_backend() -> None:
    topology = TransferTopology(
        tp_rank=0,
        tp_size=4,
        block_size=16,
        ...
        is_mamba=False,
        total_num_kv_heads=8,
        attn_backends=[_FakeIndexerBackend, _UnexpectedBackend],
    )
