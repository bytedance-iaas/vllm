...
def test_scheduler_request_finished():
    ...
    scheduler_connector._online_c128_state_transfer_enabled = False
...
def test_register_kv_caches_skips_speculative_layers_outside_base_model():
    ...
    layer_names = [
        "model.layers.0.self_attn",
        f"model.layers.{num_hidden_layers}.attn.swa_cache",
    ]
    ...
    worker._layer_specs = dict.fromkeys(layer_names, layer_spec)
    worker._layer_group_indices = dict.fromkeys(layer_names, 0)
    worker._layer_logical_group_indices = {name: [0] for name in layer_names}
