...
@pytest.mark.asyncio
async def test_bootstrap_server_waits_for_complete_tp_pp_topology():
    server = MooncakeBootstrapServer(
        "127.0.0.1",
        get_open_port(),
        expected_tp_size=2,
    ...
    payloads = [
        RegisterWorkerPayload(
            ...
            tp_rank=tp_rank,
            pp_rank=pp_rank,
            addr=f"tcp://worker-{tp_rank}-{pp_rank}:1234",
        )
    ...
    await server.register_worker(payloads[-1])
    result = await server.query()
    assert result[0].worker_addr == {
        0: {0: "tcp://worker-0-0:1234", 1: "tcp://worker-0-1:1234"},
        1: {0: "tcp://worker-1-0:1234", 1: "tcp://worker-1-1:1234"},
    }
...
def test_prefill_connector_counts_canonical_pcp_workers(
    ...
    parallel_config.prefill_context_parallel_size = pcp_size
    ...
    assert parallel_config.world_size == 4 * pcp_size
