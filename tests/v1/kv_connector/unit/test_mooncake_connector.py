...
def test_region_transfer_policy_only_reshards_gqa(layer_name, spec, expected):
    worker = MooncakeConnectorWorker.__new__(MooncakeConnectorWorker)
    worker._layer_specs = {layer_name: spec}
    worker.transfer_topo = SimpleNamespace(total_num_kv_heads=4)
    worker.vllm_config = SimpleNamespace(
        compilation_config=SimpleNamespace(
            static_forward_context={
                layer_name: SimpleNamespace(total_num_kv_heads=4)
            }
        )
    )
    region = TransferRegion(layer_name, 0, 0, 4096, 4096)

    assert worker._get_region_transfer_policy(region) == expected


def test_head_resharding_validates_gqa_region_in_hybrid_model():
    worker = MooncakeConnectorWorker.__new__(MooncakeConnectorWorker)
    worker.use_mla = True
    worker.kv_cache_config = SimpleNamespace(has_mamba_layers=True)
    worker.tp_size = 1
    worker.transfer_topo = SimpleNamespace(total_num_kv_heads=4)
    gqa_spec = SimpleNamespace(
        page_size_bytes=256,
        unpadded_page_size_bytes=128,
        kv_quant_mode=SimpleNamespace(is_nvfp4=False),
    )
    mamba_spec = SimpleNamespace()
    worker._layer_specs = {"mamba": mamba_spec, "gqa": gqa_spec}
    regions = [
        TransferRegion("mamba", 0, 0, 4096, 4096),
        TransferRegion("gqa", 1, 4096, 4096, 4096),
    ]

    with patch(
        "vllm.distributed.kv_transfer.kv_connector.v1.mooncake."
        "mooncake_connector._spec_transfers_unique_kv_heads",
        side_effect=lambda spec: spec is gqa_spec,
    ):
        error = worker._validate_head_resharding_layout(8, regions)
...
def test_sender_plan_gqa_replicas_tp1_to_tp8(remote_tp_rank, expected_src_offset):
...
def test_sender_plan_wrapper_preserves_fully_replicated_bypass():
    worker = MooncakeConnectorWorker.__new__(MooncakeConnectorWorker)
    worker.tp_rank = 2
    worker.tp_size = 8
    worker.transfer_topo = SimpleNamespace(
        local_replicates_kv_cache=True,
        total_num_kv_heads=4,
    )

    assert worker._get_sender_transfer_plan(
        local_kv_block_len=4096,
        remote_kv_block_len=4096,
        remote_tp_rank=0,
        remote_tp_size=1,
...
def test_completion_metadata_keeps_distinct_transfer_generations():
    metadata = MooncakeCompletionMetadata(finished_recving={("req", "old-transfer"): 1})
...
@pytest.mark.parametrize(
    ...
    [
        (2, 1, "producer DCP"),
        (1, 2, "consumer DCP requires global page identity"),
    ],
)
...
async def test_build_transfer_params_scatters_consumer_dcp_pages(
    ...
    assert transferred_reqs == {"d-req-dcp"}
    assert set(transferred_region_ranges) == {"d-req-dcp"}
    assert src_ptrs == [
...
async def test_build_transfer_params_combines_full_temporal_pages_and_tp_slicing(
    ...
    assert err_reqs == []
    assert err_msg is None
    assert transferred_reqs == {"d-req-draft"}
    assert transferred_region_ranges == {
        "d-req-draft": {(remote_base, 0, 0, 128)}
    }
    assert src == [
...
@pytest.mark.asyncio
async def test_consumer_dcp_rank_without_owned_pages_requires_no_region_coverage():
    layer_name = "model.layers.0.self_attn"
    layer_spec = FullAttentionSpec(
        block_size=128,
        num_kv_heads=8,
    ...
    worker.registered_group_indices = [0]
    worker.registered_layer_aliases = [[layer_name]]
    worker.registered_layer_index_aliases = [[0]]
    worker.registered_logical_group_indices = [[0]]
    worker.registered_alias_group_indices = [[[0]]]
    ...
    sent_meta = worker._xfer_meta_decoder.decode(socket.send.await_args.args[0])
    assert sent_meta.req_blocks["d-req"] == ("xfer", [[]])
    assert sent_meta.req_global_page_ids["d-req"] == [[]]
    assert not pull_meta.requires_kv_transfer
    assert pull_meta.required_region_bytes == {}
...
def test_legacy_pull_response_is_allowed_only_without_shared_aliases(
    ...
    worker = _make_pull_result_worker(requires_alias_protocol=requires_alias_protocol)
    pull_meta = PullReqMeta(
        d_req_id="d-req",
        transfer_id="xfer",
        local_block_ids=[[10]],
...
async def test_bootstrap_server(bootstrap_server: MooncakeBootstrapServer):
    ...
    payload_pcp_mismatch = {
        ...
        "pcp_size": 2,
        "cp_block_pairing_version": MOONCAKE_CP_BLOCK_PAIRING_VERSION,
        "addr": "tcp://4.4.4.4:4444",
    }
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{base_url}/register", json=payload_pcp_mismatch)
        assert response.status_code == 400
        assert "PCP size mismatch" in response.text
...
@pytest.mark.asyncio
async def test_handle_new_engine_id_marks_missing_remote_engine_failed():
    worker = MooncakeConnectorWorker.__new__(MooncakeConnectorWorker)
    ...
    worker.finished_recving_reqs = set()
    worker.finished_recving_transfer_counts = {}
    worker.receive_kv = MagicMock()
    pull_meta = PullReqMeta(
        d_req_id="d-req-bootstrap-missing",
        transfer_id="xfer-bootstrap-missing",
        local_block_ids=[[100, NULL_BLOCK_ID], [200]],
        remote_engine_id="missing-engine",
        remote_bootstrap_addr="http://bootstrap:33333",
    )

    await worker.handle_new_engine_id(
        "missing-engine", {pull_meta.d_req_id: pull_meta}
    )

    worker._connect_to_prefiller_bootstrap.assert_awaited_once_with(
        "http://bootstrap:33333"
    )
    worker.receive_kv.assert_not_called()
    assert pull_meta.pull_failed
    assert worker.finished_recving_reqs == {pull_meta.d_req_id}
    assert worker.finished_recving_transfer_counts == {
        (pull_meta.d_req_id, pull_meta.transfer_id): 1
    }
    assert worker.get_block_ids_with_load_errors() == {100, 200}


@pytest.mark.parametrize(
    ("consumer_pp_size", "producer_pp_size"),
    [(2, 1), (4, 2)],
)
def test_receive_kv_rejects_consumer_pp_fanout(
    consumer_pp_size: int, producer_pp_size: int
):
    worker = MooncakeConnectorWorker.__new__(MooncakeConnectorWorker)
    worker.shutdown = MagicMock()
    worker.pp_size = consumer_pp_size
    worker.pp_rank = 0
    worker.pcp_size = 1
    worker.pcp_rank = 0
    ...
    worker._remote_agents = {
        "p-engine": {
            0: {
                pp_rank: f"tcp://producer-pp{pp_rank}:1234"
                for pp_rank in range(producer_pp_size)
            }
        }
    }
...
@pytest.mark.asyncio
async def test_failed_send_attempt_clears_inflight_count():
    worker = MooncakeConnectorWorker.__new__(MooncakeConnectorWorker)
    worker.shutdown = MagicMock()
    worker.tp_rank = 0
    worker.tp_size = 1
    ...
    worker.transfer_topo = SimpleNamespace(
        handshake_target_ranks=lambda _remote_tp_size: [0],
        total_num_kv_heads=None,
    )
    worker._encoder = msgspec.msgpack.Encoder()
    worker._xfer_resp_decoder = msgspec.msgpack.Decoder(MooncakeXferResponse)
    worker._validate_head_resharding_layout = MagicMock(return_value=None)
    worker._producer_cache_is_replicated = MagicMock(return_value=False)
    send_meta = SendBlockMeta(
        p_req_id="p-req",
        transfer_id="xfer",
        local_block_ids=[[1]],
    ...
    with patch.object(
        ...
        AsyncMock(
            return_value=([], [], [], ["d-req"], "build failed", set(), {})
        ),
    ):
        await worker.send_kv_to_decode(b"id", socket, metadata)

    assert send_meta.sending == 0
    assert send_meta.sent == 0
    assert send_meta.transfer_id in worker.reqs_need_send
    _, payload = socket.send_multipart.call_args[0][0]
    response = worker._xfer_resp_decoder.decode(payload)
    assert response.err_reqs == ["d-req"]
...
def test_pd_trace_lifecycle_is_transfer_generation_scoped(monkeypatch):
    ...
    assert (new_meta.d_req_id, new_meta.transfer_id) in (worker._pd_trace_pull_started)
...
async def test_worker_get_finished_timeout(monkeypatch):
    ...
    with set_current_vllm_config(vllm_config), patch_worker_dependencies():
        ...
        assert completion_meta.finished_sending == {("p-req-expired", "tx-expired"): 1}
...
def test_register_kv_caches_keeps_replicated_eagle3_draft_layer():
    num_hidden_layers = 32
    worker = MooncakeConnectorWorker.__new__(MooncakeConnectorWorker)
    worker.use_mla = False
    worker.model_config = SimpleNamespace(
    ...
    worker.engine.batch_register_memory.assert_called_once_with(
        [normal_cache.data_ptr(), draft_cache.data_ptr()],
        [normal_cache.nbytes, draft_cache.nbytes],
    )
    assert worker.registered_layer_names == [normal_layer, draft_layer]
