...
struct PersistentTopKWithPageTableParams : PersistentTopKParams {
  ...
  const bool* __restrict__ valid_token_mask;  // [num_rows]
  int32_t* __restrict__ topk_lens;           // [num_rows]
  uint32_t block_table_stride;
  uint32_t page_block_size;
};

__device__ __forceinline__ int32_t local_to_physical_slot(
  ...
  if (physical_block < 0) return -1;
  return physical_block * static_cast<int32_t>(params.page_block_size) +
         static_cast<int32_t>(block_offset);
}

template <int TopK>
__device__ __forceinline__ void transform_topk_row_to_physical_slots(
    const PersistentTopKWithPageTableParams& params, uint32_t row_idx,
    int32_t* __restrict__ row_output, uint32_t seq_len, uint32_t tx) {
  __syncthreads();
  const bool is_valid_token = params.valid_token_mask[row_idx];
  const uint32_t valid_len =
      is_valid_token
          ? (seq_len < static_cast<uint32_t>(TopK)
                 ? seq_len
  ...
  for (uint32_t i = tx; i < static_cast<uint32_t>(TopK);
       i += kThreadsPerBlock) {
    row_output[i] = is_valid_token
                        ? local_to_physical_slot(params, row_idx, row_output[i])
                        : -1;
  }
}
...
template <int TopK = 2048, uint32_t VEC_SIZE = 1,
          bool WithPageTable = false, typename Params = PersistentTopKParams>
    ...
    if (seq_len <= RADIX_THRESHOLD) {
      if (cta_in_group == 0) {
        ...
        if constexpr (WithPageTable) {
          transform_topk_row_to_physical_slots<TopK>(
              params, row_idx, row_output, seq_len, tx);
        }
      }
      continue;
    }
