...
void ValkeyClient::set(std::string key, torch::Tensor& value) {
  ...
  void* reply = valkeyCommand(this->swap_out_valkey_ctx, "SET %b %b", key.c_str(),
                              key_len, value_ptr, value_len);
  valkeyReply* valkey_reply = static_cast<valkeyReply*>(reply);
  assertReplyAndFree(this->swap_out_valkey_ctx, valkey_reply, VALKEY_REPLY_STATUS);
  return;
}
