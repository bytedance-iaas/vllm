...
#
# Optional:
#   DEEPGEMM_PYTHON_VERSIONS (space-separated versions; defaults to every
#     version allowed by pyproject.toml)
#   DEEPGEMM_VENV_PREFIX (default: /tmp/dgenv)
#   DEEPGEMM_UV_VENV_TIMEOUT (default: 600s)
set -euo pipefail

if [ "$#" -eq 0 ] && [ -n "${DEEPGEMM_PYTHON_VERSIONS:-}" ]; then
  read -ra versions <<< "${DEEPGEMM_PYTHON_VERSIONS}"
  set -- "${versions[@]}"
elif [ "$#" -eq 0 ]; then
