...
MEGA_WRAPPER_CALLS = {
    ...
    "get_symm_buffer_for_mega_moe": (
        (
            "DeepSeek V4 SM90 dispatch",
            6,
            frozenset({"use_fp8_dispatch", "activation"}),
        ),
    ),
    "transform_weights_for_mega_moe_sm90": (("DeepSeek V4 SM90 FP8", 2, frozenset()),),
    "transform_weights_for_mega_moe_sm90_fp4": (
        ("DeepSeek V4 SM90 FP4", 2, frozenset()),
    ),
}
REQUIRED_CPP_TOKENS = {
    "csrc/python_api.cpp": {
...
CPP_BINDINGS = {
    "csrc/apis/layout.hpp": {
        "transform_sf_into_required_layout": (
            "transform_sf_into_required_layout",
            (
                ("DeepSeek V4 SM100/Kimi default", 5, frozenset()),
                ("DeepSeek V4 SM90 FP8", 5, frozenset({"disable_ue8m0_cast"})),
            ),
        ),
    },
...
def check_source(source_dir: Path) -> list[str]:
    ...
    try:
        ...
        for name, call_variants in MEGA_WRAPPER_CALLS.items():
            function = wrappers.get(name)
            if function is None:
                continue
            for label, num_positional, keyword_names in call_variants:
                if _accepts_call(function, num_positional, keyword_names):
                    continue
                keywords = ", ".join(sorted(keyword_names)) or "none"
                errors.append(
                    f"{mega_path}: incompatible wrapper signature for "
                    f"{name} ({label}): requires {num_positional} "
                    f"positional arguments and keywords {keywords}"
                )
    ...
    for relative_path, bindings in CPP_BINDINGS.items():
        ...
        for api_name, (symbol_name, required_call) in bindings.items():
            ...
            if required_call is not None:
                ...
                for label, num_positional, keyword_names in required_call:
                    if _accepts_named_call(
                        parameter_names,
                        default_names,
                        num_positional,
                        keyword_names,
