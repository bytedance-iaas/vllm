...
- benchmark 原始默认使用 evalscope；vLLM 自带 `vllm bench serve` / `vllm.benchmarks.serve` 必须按命令引用作为对照并解释口径差异。2026-07-02 用户已明确接受 `vllm bench serve` 结果作为当前发布性能 gate，因此后续 M10 gate 以同一个新构建 vLLM 镜像内、no-GPU benchmark Pod、in-cluster router Service、BS512/2048 requests、64K prefix、1536 output 的 `vllm bench serve` Avg 结果为准。vLLM 自带压测必须在同一个新构建 vLLM 镜像启动的容器内执行，不能用本地工作站 Python 环境；该容器不请求 GPU，不做运行时安装、代码 clone 或 hotfix，通过集群内 router Service 发请求，结束后删除。
- evalscope 与 vLLM bench 结果出现显著差异时，必须先按命令引用 `C21W` 做差异分析；分析对象包括请求构造、prefix cache 命中语义、client 网络路径、并发/限流模型、超时与失败处理、统计口径、Prometheus running/waiting/output TPS、Mooncake/KV 错误窗口和服务状态延续。`C21W` 未完成或未能解释差异前，不得把 vLLM bench 的 BS512 成功结果作为替代 gate；如离线 artifacts 不能解释原因，才执行 `C21X` 重新部署做最小配对复现实验。当前 C21W/C23/C27/C28 已完成并解释差异，且用户在 2026-07-02 接受 vLLM benchmark gate；历史 evalscope 不稳定不再阻塞当前 `iaas_main` 发布判断，但必须保留为风险和后续诊断项。
...
- 用户在 2026-07-01 进一步要求解释 decode 场景下 evalscope TTFT 为什么高于 `vllm bench serve`。新增 `C27` 必须专门分析 TTFT 差异，而不是只复述 throughput gate；分析范围包括：evalscope request generation 是否进入统计窗口、首批/末批请求 TTFT、router/admission/waiting、decode running BS 维持能力、prefill/KV transfer 触发点、client 连接/HTTP streaming 行为、timeout/失败请求处理、统计口径差异、以及 `vllm:spec_decode_*` 是否支持投机解码差异。`C27` 首先使用已有 artifacts 离线分析，不创建 GPU workload；只有离线证据无法解释时才规划 live paired run，且 live run 必须重新获取 workspace-env permit。
- 2026-07-01 用户指出不能因 M10 发布 blocked 停止 TTFT 根因分析。新增 `C28` 作为 C27 的 live 证据补强：在同一部署语义和同一镜像下，重新部署 `1P1D`，运行 in-cluster evalscope BS512/2048 与 in-cluster `vllm bench serve` BS512/2048 配对实验，并在每个 measured window 抓取 decode running/waiting/output TPS、`vllm:spec_decode_*`、TTFT/TPOT 指标、bad-log、MooncakeDiag 和 client/request timeline。`C28` 当时不改变发布 gate、不更新 `iaas_main`、不修改源码；2026-07-02 用户基于 C28 结论接受 vLLM benchmark 作为当前发布 gate。
...
### M9: benchmark 64k/1 TTFT 和 cache-hit decode BS512/1.5k output
...
- Acceptance: measured run 均有开始/结束时间、exit code、raw output、artifact 路径、TTFT/throughput 摘要；性能 gate 看 Avg，不看 P50/P95/P99。64k/1 Avg TTFT 必须小于 10s，当前有效 C20 结果满足该要求。BS512/1.5k decode throughput 当前发布 gate 使用用户 2026-07-02 接受的 `vllm bench serve` 结果：同一新构建 vLLM 镜像、in-cluster router Service、`--max-concurrency 512`、`--num-prompts 2048`、64K prefix、1536 output、`ignore_eos`，output throughput Avg 必须达到 14000 tokens/s 以上，Prometheus 需证明 decode running 接近目标 BS，且 bad-log scan clean；该 throughput 口径为 `1P1D` router-path，总 output 来自单个 decode 节点。evalscope C21/C21A/C24/C26/C28 结果继续作为风险和根因证据保留，不再作为当前 M10 阻塞 gate；若后续重新改回 evalscope gate，仍需先重启服务并按 C21/C21A/C21M/C21R 执行。差异分析 gate 已完成：`C21W` 产出 `artifacts/2026-06-29-vllm-dsv4-flash-pd/harness-diff-analysis-20260701/summary.md`；Mooncake/KV failure 初筛已完成：`C23` 产出 `artifacts/2026-06-29-vllm-dsv4-flash-pd/mooncake-failure-diagnosis-20260701/summary.md`；C27/C28 已解释 evalscope TTFT/throughput 波动主因。
...
- [ ] benchmark 跑通但性能未达当前发布阈值时也不得更新远端 `iaas_main`；本计划性能 gate 看 Avg，不看 P50/P95/P99。2026-07-02 用户已接受 vLLM benchmark 结果并认为总体通过，因此当前 M10 gate 为：64k/1 Avg TTFT < 10s；`1P1D` router-path `vllm bench serve` BS512/1.5k output throughput >= 14000 tokens/s；BS512 gate 必须来自 `num-prompts=2048` 且 Prometheus 证明实际 running BS 接近目标的结果，降档 BS 通过只能作为容量诊断，不能替代 BS512 gate。
...
- [x] 如果 `C24` 证据不足，按命令引用 `C25` 添加最小调试代码并推送诊断分支；调试代码只允许在 MooncakeConnector producer/consumer transfer 路径记录 batch descriptors、bytes、request count、remote session、role/rank、elapsed、ret、pending/expired request count 和可用 transfer config，不改变 runtime 语义。调试代码必须默认可控，例如通过 `VLLM_DSV4_MOONCAKE_DIAG=1` 或等价 env gate 开启。
- [x] `C25` 构建调试镜像后复测时，部署模板只允许增加诊断 env，不能更改 Mooncake 版本、IB 设备、TP/PP/DP/EP、maxNumSeqs、maxNumBatchedTokens、maxModelLen、MTP、DeepEP 或 router 语义。
...
- [x] 按命令引用 `C27` 专项分析 decode 场景下 evalscope Avg TTFT 为什么高于 `vllm bench serve`，优先离线复用 C24/C25/C26B/C26C artifacts，不创建 GPU workload、不修改源码、不更新 gate。
- [x] `C27` 必须交叉对比 C24 同服务窗口、C26B in-cluster 路径、C26C 降档 sweep 和 C25 调试镜像结果，区分 request generation 是否进入统计、首批/末批请求 TTFT、router/admission/waiting、decode running BS、server-side output TPS、prefill/KV transfer、client connection/HTTP streaming、失败请求处理和统计口径。
- [x] C27 已给出离线结论，但缺少同一 live window 下 vLLM bench 的 server-side `vllm:spec_decode_*` 和更强的 request/admission timeline，因此不能把 speculative decoding、HTTP/client path 或 admission 差异定成 definitive。
- [x] 按命令引用 `C28` 执行 live TTFT 相关性实验：重新获取 workspace-env permit，使用 C25 调试镜像和相同 `1P1D` 语义，只增加 `VLLM_DSV4_MOONCAKE_DIAG=1`，运行 in-cluster evalscope BS512/2048 与 in-cluster `vllm bench serve` BS512/2048，并在每个窗口抓取 decode running/waiting/output TPS、server-side `vllm:spec_decode_*`、vLLM TTFT/TPOT metrics、MooncakeDiag/bad-log 和 client timeline。
- [x] `C28` 必须在失败或中断后重启 P/D/router，避免坏状态污染下一轮；结束后清理 serving/monitoring/benchmark Pod/port-forward，并释放 permit。
- Acceptance: C26 summary 必须明确 evalscope 未达标的主因分类、speculative decoding 证据状态、是否需要 C25 调试代码、是否改变 M10 gate。C27 summary 必须回答 decode 场景 evalscope Avg TTFT 高于 vLLM bench 的原因，按 `confirmed`、`likely`、`inconclusive` 分级列出证据，并覆盖 first/last request TTFT、Prometheus running/waiting、server-side spec、client path、request generation、统计口径和 Mooncake/RDMA evidence。C28 summary 必须给出同一 live 服务窗口下 evalscope 与 vLLM bench 的 Avg TTFT、first/last request TTFT、decode running/waiting、server-side spec accept proxy、output TPS、bad-log 与 client timeline 对比，并明确 C27 的 likely 结论是否被 live 证据确认、推翻或仍然 inconclusive。当前 C26/C27/C28 已完成；2026-07-02 用户接受 vLLM benchmark 结果，因此 vLLM bench 不再只是 harness cross-check，而是当前发布性能 gate。evalscope 波动保留为风险，不再阻止 M10。
...
- evalscope 与 vLLM bench 的显著差异已完成 `C21W`、`C23`、`C27`、`C28` 分析，并保存可审计结论；当前差异已解释为服务状态、admission/running/waiting、client/request scheduling 与 Mooncake/RDMA 间歇性放大共同影响。用户在 2026-07-02 已接受 vLLM bench BS512 成功结果作为当前发布性能 gate。
- evalscope BS512 Mooncake/KV failure 已完成 C23 离线初筛；后续必须通过 M11/C24 判断是否稳定复现、偶现、harness-specific 或需要调试镜像，才能继续推进发布判断。
- evalscope 与 vLLM bench 的 live 成功结果差异已完成 C26/C28 深挖；C28 已保存 server-side `vllm:spec_decode_*` proxy 证据，当前不支持 speculative decoding 接受率是主要差异点。不得仅凭 vLLM bench JSON 推断接受率；如后续重新调查，继续使用 server-side 指标。
- decode 场景 evalscope Avg TTFT 高于 vLLM bench 的专项 C27/C28 分析必须完成或明确阻塞；结论必须说明更高 TTFT 是否来自客户端请求构造/发送、server admission/running BS 不稳定、prefill/KV transfer、HTTP streaming/连接复用、失败请求纳入统计、统计口径差异、投机解码接受率差异或证据不足。C27 离线结论不足以停止 goal；如果继续分析根因，必须用 C28 获取同窗口 live 指标补强。
...
- 可直接执行的已授权动作：
  ...
  - 2026-07-02 用户已追加决策：接受 `vllm bench serve` 结果，认为总体通过；后续可按当前发布 gate 推进 M10，不再因历史 evalscope 波动单独阻塞。
...
- vLLM 自带 `vllm bench serve` 支持 `--request-rate inf`、`--max-concurrency`、`--num-prompts`、`--random-prefix-len`、`--random-output-len` 和 `--ignore-eos`；其统计口径、prefix 构造、tokenizer 对齐和并发限流与 evalscope 不完全相同。C21V/C24/C28 已证明 vLLM bench 可在同一镜像和 in-cluster router path 下把 decode running 推近 BS512 并达到用户吞吐阈值；用户在 2026-07-02 接受其作为当前发布 gate。C21V/C28 必须保留 artifact，后续如果重跑仍必须在同一个新构建 vLLM 镜像启动的临时 benchmark 容器内运行；本地 Python 缺依赖不作为验证结果。
- evalscope 和 vLLM bench 结果差异很大：evalscope 多轮 BS512/2048 出现 invalid、低吞吐或明显 TTFT 波动，vLLM bench BS512/2048 多次成功且 throughput 超过 `14000 tok/s`。C21W/C23/C27/C28 已完成归因：差异与服务状态、admission/running/waiting、client/request scheduling、统计窗口和 Mooncake/RDMA 间歇性放大相关，不支持 speculative decoding 接受率是主因。该风险不再阻塞当前 M10，但后续若线上或日构建监控出现类似 Mooncake/KV timeout，应单独按诊断计划跟进。
...
- speculative decoding 接受率分析存在 artifact 缺口：evalscope summary 有 `Spec. Accept Rate`，C25 有 evalscope 路径下的 server-side spec counter 代理证据，但 vLLM bench JSON 没有 spec accept/draft 字段，且缺少同一服务状态下 vLLM bench 的 `vllm:spec_decode_*` 完整采样。因此当前只能判断“没有证据表明 evalscope 接受率更差”，不能宣称两者接受率完全相同。若该方向成为发布阻塞点，必须重跑 C26B 采集 server-side 指标。
...
- benchmark 性能未达当前发布阈值也必须阻止远端 `iaas_main` 更新；本计划性能 gate 看 Avg，不看 P50/P95/P99。2026-07-02 后的当前发布阈值为：64k/1 Avg TTFT < 10s；`1P1D` router-path vLLM bench BS512/1.5k output throughput >= 14000 tokens/s；BS512 measured run 必须使用 `num-prompts=2048`，Prometheus 需显示 decode running 接近目标，bad-log scan clean。

## Current Status

- 当前工作区分支：`codex/vllm-dsv4-mooncake-transfer-diagnostics`，HEAD `a51e395f7`，是 C25 诊断分支；它基于 fork SHA `cde7799cc66c5a4cb349156a3ca3228f9798dbc9`，但包含默认关闭的 Mooncake 诊断日志改动，不能直接作为远端 `iaas_main` 发布依据。原集成/发布基线仍以 `codex/vllm-dsv4-fork-base-byteiaas-build` 和成功构建 run `28442949331` / run `28452612809` 的非诊断镜像为准；若后续要恢复发布判断，必须先回到非诊断集成分支重新核对 diff scope。
...
- 历史 M10 决策曾因 evalscope gate 未达标而不执行；该决策已被 2026-07-02 用户新口径更新覆盖，当前不再以历史 evalscope 波动单独阻止 M10。
...
- C25 调试镜像已构建成功：诊断分支 `codex/vllm-dsv4-mooncake-transfer-diagnostics` 添加默认关闭的 `VLLM_DSV4_MOONCAKE_DIAG` 日志开关，仅记录 producer/consumer transfer 诊断信息，不改变 transfer、timeout、调度、retry、fallback import 或部署语义；ByteIAAS dev workflow run `28511013026` 成功，调试镜像为 `iaas-gpu-cn-beijing.cr.volces.com/serving/vllm:v0.10.0.iaas.dev.202607011829-openai-devel-cu130`，digest `sha256:467818239e1d896f5b51fe7b92aaade56626077974cc0143559b0b6bc6f33ca3`。后续复测只允许在同一部署语义上增加 `prefill.extraEnv.VLLM_DSV4_MOONCAKE_DIAG=1` 和 `decode.extraEnv.VLLM_DSV4_MOONCAKE_DIAG=1`。详见进展日志 `P82`-`P86`。
- C25 调试镜像复测已完成并清理：使用调试镜像、prefill `192.168.1.186`、decode/router `192.168.1.154`、同一 `1P1D` 语义且仅增加 `VLLM_DSV4_MOONCAKE_DIAG=1`。in-cluster no-GPU evalscope Pod 通过代理安装 `evalscope[perf]==1.8.1` 并完成 BS512/2048、64k prefix、1536 output formal run：`2048 / 2046 / 2`，Avg TTFT `22313.47 ms`，Avg output throughput `12688.88 tok/s`，未达 gate；Workload `Completion tok/s` Overall `12750.25`、Last 30s `20147.95`、Steady drop 20% `17236.92`，但计划 gate 仍看 evalscope Overall/Avg，不能以 Last 30s 或 steady 替代。Prometheus in-pod 快照显示处理中 decode running 约 `392`，未长期达到 512；30s decode generation rate 合计约 `16.3k tok/s`，server-side spec 指标与 evalscope `Spec. Accept Rate=0.78` 同向。详见进展日志 `P87`。
- evalscope 安装策略已统一：本地 C17 和所有 in-cluster benchmark Pod 均必须使用代理 `http://100.68.170.29:3128` 安装 `evalscope[perf]==1.8.1`；Pod 内命令固定为 `python3 -m pip install --proxy http://100.68.170.29:3128 -U 'evalscope[perf]==1.8.1'`，并保留 `.svc/.cluster.local` 等 `NO_PROXY`，避免 benchmark 请求绕代理访问集群内 router。
- C27 离线专项分析已完成：evalscope 与 vLLM bench 的 TTFT 公式相近，request generation 不直接计入单请求 TTFT；evalscope 更高 Avg TTFT 的主要原因是 closed-loop 客户端发射形态、大 64K JSON 请求上传/连接行为、router/prefill/decode admission 与 waiting 共同导致首 token 前排队更重，服务端 decode running 平均值更低且 waiting 更高。TPOT/ITL 不支持 decode token 生成更慢作为主因；投机解码接受率缺少同窗口 vLLM bench `vllm:spec_decode_*`，不能 definitive 对齐；Mooncake/RDMA timeout 是间歇性放大因素。详见 artifact `decode-ttft-divergence-analysis-20260701/summary.md` 和进展日志 `P89`。
- 2026-07-01 计划状态复核：M2-M7 中仍有早期未勾选的子项，但后续 Current Status、P38-P46、P55-P89 已记录实际完成和验证证据；这些空 checkbox 不能被解释为需要从 C3/C4/C13 重新开始。需要注意的是，当前工作区处在诊断分支，`git diff <fork_sha>...HEAD` 会包含 C25 诊断 runtime 文件，因此 M2/M3 的“发布前 diff scope”验收必须在非诊断集成分支上重新核对，不能用当前诊断分支直接通过。详见进展日志 `P90`。
- 2026-07-01 修正执行判断：M10 发布仍 blocked，但 TTFT 根因分析不应因此停止。计划新增 C28 live TTFT 相关性实验，用于补齐 C27 缺少的同窗口 server-side spec/admission/client timeline 证据；C28 不修改源码、不改变部署语义、不更新 gate。详见进展日志 `P91`。
- C28A/C28B 已完成：GPU permit `f2483ba8-9d26-4790-8b76-bb137d2513d7` 已获取并标记 running；使用 C25 调试镜像 `v0.10.0.iaas.dev.202607011829-openai-devel-cu130` 重新部署同一 `1P1D` 语义，prefill 在 `192.168.1.186`，decode/router 在 `192.168.1.149`，仅增加 `VLLM_DSV4_MOONCAKE_DIAG=1`，router `/v1/models` 与真实 completion smoke 通过。详见进展日志 `P92`。
- C28C/C28M 已完成：initial paired run 中 evalscope `2048/2048` 成功但 Avg TTFT `27747.15 ms`、output throughput `11564.47 tok/s` 未达 gate；同一服务后续 `vllm bench serve` `2048/2048` 成功，Mean TTFT `7014.98 ms`、output throughput `15591.78 tok/s`。为补齐 first/last request TTFT，继续用 hostPath 持久化 artifact 补跑 persist pair：evalscope `2048/2048` 成功，Avg TTFT `5942.55 ms`、first512 `17949.20 ms`、last512 `1897.11 ms`、output throughput `16457.09 tok/s`；vLLM bench `2048/2048` 成功，Mean TTFT `5797.22 ms`、first512 `17474.05 ms`、last512 `1873.39 ms`、output throughput `15497.32 tok/s`。server-side spec accept proxy 接近，bad-log scan 为 0。C28 结论分类为 `intermittent/service-state-sensitive`：initial evalscope 高 TTFT 与服务端 running/waiting/admission 状态强相关；evalscope harness 本身不是稳定劣化的充分原因；cache warm-up 只能作为可能因素，不能单独确认。详见进展日志 `P93`-`P95` 与 artifact `c28-live-ttft-correlation-20260701/summary.md`。
- C28Z 已完成主要清理：serving/monitoring Helm release、benchmark Pods 已删除，permit `f2483ba8-9d26-4790-8b76-bb137d2513d7` 已释放；两个任务 namespace 删除后处于 Kubernetes `Terminating` 中，需后续按 C28Z cleanup artifact 复查最终消失。详见进展日志 `P95`。
