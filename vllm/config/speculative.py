...
class SpeculativeConfig:
    ...
    """DP-safe Dynamic SD policy. ``global_max`` uses global batch pressure."""
    dynamic_sd_dp_sync_interval: int = Field(default=8, ge=1)
    """Number of DP engine steps between global pressure synchronizations."""
            ...
            for _, _, num_speculative_tokens in sorted(
                ...
                if last_k is not None and current_k > last_k:
                    raise ValueError(
                        ...
                        "num_speculative_tokens_per_batch_size to be "
                        "non-increasing as batch size grows."
                    )
                last_k = current_k
