        ...
        if self.method in ("ngram", "ngram_gpu"):
            ...
            if self.model is not None:
                ...
                if self.method == "dspark":
                    ...
                    # layout and yields incorrect output rather than merely
                    # lower acceptance. Runtime Dynamic SD K values remain
                    # separately bounded by scheduler/runtime logic.
                    ...
                    if (
                        dspark_block_size is not None
                        and self.num_speculative_tokens_per_batch_size is not None
                    ):
                        invalid_runtime_k = [
                            num_speculative_tokens
                            for _, _, num_speculative_tokens in (
                                self.num_speculative_tokens_per_batch_size
                            )
                            if 0 < num_speculative_tokens < dspark_block_size
                        ]
                        if invalid_runtime_k:
                            raise ValueError(
                                "DSpark dynamic speculative decoding requires every "
                                "nonzero scheduled num_speculative_tokens to be >= "
                                f"dspark_block_size ({dspark_block_size}); got "
                                f"{sorted(set(invalid_runtime_k))}."
                            )
    ...
    def _verify_args(self) -> Self:
        ...
        if self.draft_model_config:
            if self.enable_eagle3_prefill_draft_kv:
                ...
                if pp_size > 1 and not current_platform.is_cuda():
                    raise NotImplementedError(
