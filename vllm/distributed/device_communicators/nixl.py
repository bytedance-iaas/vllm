...
class DynamoNixlConnector:
    def __init__(self, vllm_config: VllmConfig, engine_id: str, rank: int,local_rank:int ,world_size:int):
    ...
    def register_kv_caches(self, kv_caches: List[torch.Tensor]):
        ...
        if self.use_mla:
            for key_cache in kv_caches:
                ...
                caches_data.append((base_addr, region_len, self.rank, "",self.world_size,self.local_rank))
            ...
            for layer_id in layer_ids:
                if self.use_mla:
                    ...
                    for block_id in block_ids:
                        ...
                        descs_ids.append(layer_id * num_blocks * tp_multiplier + start_offset * tp_multiplier + i_offset + (block_id - start_offset))
                else:
                    for is_value in [0, 1]:
                        staging_range_idx = 0
                        for block_id in block_ids:
        ...
        for local_range, staging_range in zip(local_rearranging_ranges, staging_rearranging_ranges):
            ...
            for kv_cache in self.kv_caches:
                if self.use_mla:
                    ...
                    rearrange_tensors(cache_src, cache_staging, 1, "read") # for mla, the head size is not the same as flash attention, split not required here
                else:
    ...
    def add_remote_agent(self, engine_id, agent_metadata, agent_tp, kv_caches_base_addr, num_blocks):
        ...
        for i in range(tp_multiplier):
            ...
            descs = self.nixl_wrapper.get_xfer_descs(blocks_data, "VRAM")
            logger.debug(f"self.rank * tp_multiplier + i {self.rank * tp_multiplier + i}")
