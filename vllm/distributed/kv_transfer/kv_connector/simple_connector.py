...
class SimpleConnector(KVConnectorBase):
    ...
    def send_kv_caches_and_hidden_states(
        ...
        # mla
        elif isinstance(model_input.attn_metadata, TritonMLAMetadata):
            for idx, slen in enumerate(seq_lens):
                start_pos = sum(seq_lens[:idx])
                end_pos = start_pos + slen
                current_tokens = input_tokens_tensor[start_pos:end_pos]
                ...
                for layer_id in range(start_layer, end_layer):
                    ...
                    keys.append(kv_cache[current_slot_mapping].unsqueeze(0))

                keys = torch.cat(keys, dim=0)
                values = torch.zeros(1) # dummy value
    ...
    def recv_kv_caches_and_hidden_states(
        ...
        # mla
            ...
            for idx, slen in enumerate(seq_lens):
                ...
                roi: torch.Tensor = ret[1]
                keys: torch.Tensor = ret[2]
                values: torch.Tensor = ret[3] # dummy value
                hidden: torch.Tensor = ret[4]
