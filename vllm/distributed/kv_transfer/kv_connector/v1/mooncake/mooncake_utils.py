...
class MooncakeBootstrapServer:
    ...
    def _register_routes(self):
        ...
        self.app.get("/query", response_model=dict)(self.query)
    ...
    async def query(self) -> dict[int, dict]:
        result: dict[int, dict] = {}
        for dp_rank, entry in self.workers.items():
            result[dp_rank] = {
                "engine_id": entry.engine_id,
                "pcp_size": entry.pcp_size,
                "cp_block_pairing_version": entry.cp_block_pairing_version,
                "worker_addr": entry.worker_addr,
            }
        return result
