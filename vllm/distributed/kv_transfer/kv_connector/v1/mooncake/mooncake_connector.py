...
def _shared_alias_group_pairs(
    ...
    local_alias_groups = _alias_group_map(local_region)
    remote_alias_groups = _alias_group_map(remote_region)
    pairs: set[tuple[str, int, int, int, int]] = set()
    for alias in set(local_alias_groups) & set(remote_alias_groups):
...
def _align_transfer_regions(
    ...
    matched_remote_keys: set[tuple[int, str, int, int, int]] = set()
    ...
    for local_idx, local_region in enumerate(alias_local_regions):
        ...
        for remote_idx, remote_region in enumerate(alias_remote_regions):
            ...
            available_pairs = [
                ...
                if (remote_idx, pair[0], pair[1], pair[3], pair[4])
                not in matched_remote_keys
                and (pair[0], pair[1], pair[2], pair[4]) not in matched_local_keys
            ]
            if not available_pairs:
                continue
            matched_local_keys.update(
                (alias, layer_index, local_group, region_part)
                for alias, layer_index, local_group, _, region_part in available_pairs
            )
            matched_remote_keys.update(
                (remote_idx, alias, layer_index, remote_group, region_part)
                for alias, layer_index, _, remote_group, region_part in available_pairs
            )
            aligned_local.append(local_region)
            aligned_remote.append(remote_region)
    ...
    legacy_local, legacy_remote, legacy_err = _align_transfer_regions_by_occurrence(
        legacy_local_regions, legacy_remote_regions
    )
...
class MooncakeConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        is_decode_local_draft |= (
            speculative_method == "eagle3" and not transfers_eagle3_draft
        )
    ...
    def _get_required_region_bytes(
        ...
        for region in regions:
            for group_index in _region_group_indices(region, len(block_ids)):
                if not self._group_has_transferable_blocks(
    ...
    def _get_region_transfer_policy(
        ...
        num_kv_heads = next(iter(head_counts)) if head_counts else model_num_kv_heads
