...
def native_next_n_supported(next_n: int) -> bool:
    ...
    The pinned SM90 MegaMoE fork implements only {1, 2}. Newer upstream
    kernels also support 4, but that requires a matching scheduler ABI.
    Unsupported values must be flattened to one row per query.
    """
