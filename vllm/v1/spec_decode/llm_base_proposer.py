...
class SpecDecodeBaseProposer:
    ...
    def _validate_replicated_draft_kv_config(self) -> None:
        ...
        if kv_transfer_config is not None and not mooncake_consumer:
            errors.append(
                "KV transfer requires the built-in Mooncake consumer in fail mode"
            )
        if parallel_config.decode_context_parallel_size > 1 and not mooncake_consumer:
            errors.append(
                "DCP replicated draft KV requires Mooncake remote prefill in "
                "consumer fail mode"
            )
        if self.parallel_drafting:
    ...
    def _update_positions_dependent_metadata(
        ...
        if self.replicated_draft_kv:
            self._advance_replicated_draft_cpu_sequence_metadata(
                common_attn_metadata,
            )
        else:
    ...
    def _advance_replicated_draft_cpu_sequence_metadata(
        self,
        ...
        if seq_lens is not None:
            exceeds_max = seq_lens >= self.max_model_len
            seq_lens.add_(1)
            seq_lens.masked_fill_(exceeds_max, 1)
        elif upper_bound is not None:
            exceeds_max = upper_bound >= self.max_model_len
        ...
        if upper_bound is not None and not upper_bound_aliases_seq_lens:
            exceeds_upper_bound = upper_bound >= self.max_model_len
            upper_bound.add_(1)
