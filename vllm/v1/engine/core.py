...
class DPEngineCoreProc(EngineCoreProc):
    ...
    def run_busy_loop(self):
        ...
        while self._handle_shutdown():
            ...
            speculative_config = self.vllm_config.speculative_config
            dynamic_sd_sync_enabled = (
                speculative_config is not None
                and speculative_config.uses_dynamic_sd_dp_global_max_policy()
            )
            if (
                dynamic_sd_sync_enabled
                or local_unfinished_before_step
                or self.engines_running
            ):
                # Keep the old-group collective order stable while Elastic EP
                # waits for late ranks: pressure sync, EEP barrier, then worker.
            ...
            if self.eep_scaling_state is not None:
                ...
                # Group switching resets the pressure cache before EPLB reaches
                # COMPLETE. Bootstrap immediately after the synchronized group
                # transition, before any worker step uses the new group.
