...
def _prepare_uniform_decode_kernel(
    ...
    per_token_seq_len = tl.maximum(per_token_seq_len, 0)
    ...
    def _prepare_decode_tensors(
        ...
        if not use_native and max_decode_len > 1:
            ...
            if min_decode_len == max_decode_len:
                ...
                self.decode_seq_lens_buffer[:actual_expanded].clamp_(min=0)
    ...
    def build(
        ...
        if num_decodes > 0:
            ...
            if self.compress_ratio > 1:
                ...
                # DeepGEMM paged MQA metadata does not tolerate zero-length
                # dummy decode rows generated by CUDA graph profiling. Preserve
                # real short requests whose compressed KV length is legitimately
                # zero.
