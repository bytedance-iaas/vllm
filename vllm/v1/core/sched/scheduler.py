...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        if self.enable_prefill_token_bucket_schedule and self.has_mamba_layers:
            raise ValueError(
                "prefill token bucket scheduling does not support Mamba KV cache"
            )
    ...
    def _get_prefill_bucket_step_budget(
        ...
        used_budget = base_token_budget - token_budget
        return min(token_budget, max(step_bucket[1] - used_budget, 0))
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if load_kv_async:
                    ...
                    if 0 < threshold < num_new_tokens:
                        if pad_spec_decode and self._dynamic_sd is not None:
                            # Preserve the uniform padded width or defer this request.
                            break
