...
def warmup_kernels(
    ...
    use_parallel_draft_full_k_warmup = _uses_parallel_draft_full_k_warmup(model_runner)
    generic_finished_req_ids = set(req_ids)
    parallel_finished_req_ids: set[str] = set()
    parallel_warmup_attempted = False
    startup_num_spec_tokens = 0
    if use_parallel_draft_full_k_warmup:
    ...
    try:
        ...
        if use_parallel_draft_full_k_warmup:
            if generic_finished_req_ids:
                try:
                    _cleanup_warmup_requests(
                        worker_execute_model, generic_finished_req_ids
                    )
                except Exception:
            ...
            if parallel_finished_req_ids:
                try:
                    ...
                    logger.exception(
                        "Failed to clean up parallel-draft warmup request."
                    )
            ...
            model_runner.kv_connector.set_disabled(False)
    if not use_parallel_draft_full_k_warmup:
        model_runner.kv_connector.set_disabled(False)
