...
class MiniMaxM3HPCExperts(mk.FusedMoEExpertsModular):
    ...
    def _supports_parallel_config(
        ...
        return (
            moe_parallel_config.tp_size == 16
    ...
    def apply(
        ...
        assert global_num_experts == self.num_experts == 128
        assert hidden_states.dtype == torch.bfloat16
        assert hidden_states.ndim == 2 and hidden_states.size(1) == 6144
        ...
        assert workspace13 is not None and workspace13.dtype == torch.bfloat16
        assert workspace2 is not None and workspace2.dtype == torch.bfloat16
        ...
        # The grouped input is dead before activation runs on the same stream.
